...
Every SSE rule accepts the cross-transport matcher surface: `urlPattern`, `hostname`, `queryParams`, and the `matcher: 'name'` reference into a registered `NamedMatcher`. Mix and match exactly like network rules, with the same `matcher_inline_conflict` and `transport_urlpattern_required` validation guarantees.
