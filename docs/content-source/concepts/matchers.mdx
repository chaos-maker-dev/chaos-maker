...
If neither `matcher` nor `urlPattern` is set on a WS or SSE rule, validation surfaces a `transport_urlpattern_required` issue. This is stricter than network rules, where `urlPattern` is optional whenever any inline matcher field is present.
