...
The TypeScript shape `TransportRuleMatchers` is a discriminated union: a rule either declares `urlPattern` (with optional `hostname` / `queryParams` refinements) or declares `matcher: 'name'`. At runtime, the existing `matcher_inline_conflict` check still rejects rules that supply neither, so untyped configs cannot ship a rule with no targeting at all.
