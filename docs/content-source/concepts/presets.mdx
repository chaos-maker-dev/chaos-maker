...
| `aiMobileInterrupt`     | `ai-mobile-interrupt`               | fetch-stream, sse, ui | User backgrounds the app mid-generation: tab reports hidden for 3s (from 1s) while the stream drops (after chunk 10, or 4s for SSE). Assert visibility pause logic plus stream recovery. |
