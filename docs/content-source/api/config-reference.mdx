...
A rule sets either inline targeting (`urlPattern` required at the TypeScript level, with optional `hostname` / `queryParams` refinements) or a `matcher: 'name'` reference into the top-level matcher registry. Mixing the two surfaces `matcher_inline_conflict`. Omitting both surfaces the existing `matcher_inline_conflict` "rule must set either matcher reference or at least one matcher field" issue. The transport-irrelevant fields from `NamedMatcher` (`methods`, `requestHeaders`, `resourceTypes`, `graphqlOperation`) are rejected if inlined and silently ignored when carried by a referenced matcher so one matcher can target network, WebSocket, and SSE without duplication. See [Advanced matchers](../concepts/matchers) for the full semantics.
...
Every SSE rule type uses the same `TransportRuleMatchers` discriminated union as WebSocket rules: either inline `urlPattern` (with optional `hostname` / `queryParams` refinements) or a `matcher: 'name'` reference. The validation guarantees, debug attribution, and silent-ignore semantics for transport-irrelevant matcher fields match the WebSocket surface.
