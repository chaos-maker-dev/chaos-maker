...
## Streaming chaos and the `ai` namespace

Streaming transports (`fetch-stream`, `sse`, `websocket` inbound) share two
conventions that any new streaming rule MUST honor:

1. **`detail.phase` is a typed string from the `ChaosPhase` union** (`ai:*` or
   `user:*`, kebab-case lifecycle suffix). Set it on events emitted by a
   streaming interceptor so reporting / replay layers can render a chunk-level
   timeline without re-deriving from the event `type`. The canonical phase
   values shipped today (the `ai:*` streaming lifecycle set and the `user:*`
   human-interaction set in the `StreamingChaosPhase` union) MUST not be
   renamed in a non-major release.
2. **`detail.connectionId` is stable per logical connection**, generated by
   the interceptor (UUID v4 when `crypto.randomUUID` exists, monotonic counter
   otherwise) so multiplexed transports stay correlated. `detail.chunkIndex`
   is zero-based per connection and accompanies `phase` on every emitted
   chunk event.

The `ai` config slice is a top-level compiler shorthand. It is forbidden
inside preset, profile, and override slices because the compiler runs after
preset and profile expansion; nesting it would silently bypass compilation.
The translation table lives in `packages/core/src/ai/index.ts` as
`AI_TRANSLATIONS`; adding a new `AiConfig` field requires both a type entry
AND a translation row, and the drift test in
`packages/core/test/ai-compiler.test.ts` pins the two in lockstep.

When adding a new streaming chaos primitive:

1. Add the rule shape to the matching transport config slice
   (`FetchStreamConfig` / `SSEConfig` / `WebSocketConfig`).
2. Emit events with `detail.phase`, `detail.connectionId`, and
   `detail.chunkIndex` where applicable.
3. If the primitive maps to an AI knob, add an `AI_TRANSLATIONS` row and
   extend the drift test sample.
4. Update `docs/content-source/concepts/ai-streaming.mdx` and add a recipe
   if the primitive enables a new user-facing scenario.
